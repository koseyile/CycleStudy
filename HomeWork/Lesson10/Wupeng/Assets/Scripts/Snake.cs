...
public class Snake : SnakeNode
{
    ...
    问题 2 : 碰撞检测 都需要 继承 MonoBehaviour, 如果我是在脚本里 实例化 物体 到场景中，一定需要在脚本里 将继承 MonoBehaviour 的对象添加到物体上吗？

    问题 3 : 我用了 SnakeManager 的 静态单例 来管理 一条小蛇，是因为我需要在碰撞检测的时候 通过 静态单例 来调用小蛇中的方法。不知道这种做法是否合适？我不知道如何在碰撞检测的时候 不用静态变量访问到小蛇 ? 如果是多人游戏这个 碰撞检测 怎么办呢？
